...
'''
Detector[input_, detector_] := Module[{output, data = <||>, data2},
   data[["input"]] = AntiSlope2D[input];
   
   {data[["spikemask"]], data[["proc"]], data[["spikes"]]} = DespikerOne[data[["input"]], detector[["spikeThreshold"]]];

   {data[["aligned"]], data[["lineOffsets"]]} = ScanAlign[data[["proc"]], detector[["offsetThreshold"]]];

   {data[["mask"]], data[["tearOffset"]]} = TearMask[data[["aligned"]], detector[["tearHeightThreshold"]], detector[["tearLengthThreshold"]], detector[["tearGlue"]]];

   data[["rough"]] = data[["aligned"]] - data[["tearOffset"]];
   data[["creepInfo"]] = Creeper[data[["rough"]], detector[["offsetThreshold"]]];
   
   data2 = RebuilderStage1[data];
   data2 = RebuilderStage2[data2];
   data2 = RebuilderStage3[data2];
   data2 = RebuilderStage4[data2];
   
   data2[["steps"]] = 
    StepDetector[data2[["output"]], detector[["margin"]]];
   data2[["x0"]] += detector[["margin"]];
   data2
   ];
'''
